...
public class ClienteMapperServiceImplTest {
    ...
    public void testFromListClientViewSUCCESS(){
        ...
        Assert.assertEquals("*****00456", response.get().getCreditos().get(0).getNumeroCreditoOfuscado());
    }
    ...
    public void testFromListClientViewSUCCESS_LONG_CREDIT(){
        ...
        Assert.assertNotNull(response);
        Assert.assertTrue(response.isPresent());
        Assert.assertEquals("******************13246", response.get().getCreditos().get(0).getNumeroCreditoOfuscado());
    }
}